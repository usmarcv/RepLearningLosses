# from https://github.com/khornlund/pytorch-balanced-sampler/tree/master
...
def my_sampler(dataset, batch_size):
    targets = torch.tensor(dataset.targets)
    class_idxs = [torch.argwhere(targets == target).squeeze().numpy()
                  for target in torch.unique(targets)]
    return SamplerFactory().get(
        class_idxs, batch_size, math.ceil(len(dataset) / batch_size), 1, "fixed")
...
class SamplerFactory:
    ...
    def _fix_batches(self, weights, class_sizes, batch_size, n_batches):
        ...
        for _ in range(class_samples_per_batch.sum() - batch_size):
            largest_class = np.argmax(class_samples_per_batch)
            class_samples_per_batch[largest_class] -= 1
